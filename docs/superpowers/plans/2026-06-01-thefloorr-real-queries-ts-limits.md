...
- [x] **Step 1.2** Applied the `Rest`-recursion fix to `RewriteExtractRewriteOne` (both ` from ` and plain branches now recurse `Rest` via `RewriteExtractWalkQuoteAware`). `RewriteExtractWalkQuoteAware` already recurses the literal tail. Added regression `MultiExtractWithLiteral` in `tests/validation/select/quote-aware-structural.test.ts` (two `extract()` + a `'tag'` literal) — confirmed RED without the fix, GREEN with it.
- [x] **Step 1.3** `npm run typecheck` → only the known `_V26` line (450) errors; no new errors.

## Phase 2 — Quoted-alias case preservation past ~500 chars (correctness, bug #2)

- [x] **Step 2.1** Raised the `LowercaseOutsideQuotes` step cap 500 → **800** (`src/parsing.ts`). ⚠️ The plan's suggested 1500–2000 is WRONG: a cap > ~1000 exceeds TypeScript's tail-recursion-elimination limit and every query longer than ~1000 chars then errors `TS2589` (verified — cap=1500 produced ~24 TS2589 errors across `item-status-update`, `order-status-update`, `thefloorr/queries`, `analytics`). 800 is the sweet spot: it covers `Q_TeamPseInfo`'s SELECT list (last quoted output alias `"targetCurrency"` ENDS at char ~737) while staying safely under the limit. The quote-aware-bail alternative does NOT help here — the bail point (~char 500) is mid-`convert_currency(...)`, not inside a quote, and the aliases are past it. Insight: only SELECT-list output aliases become case-sensitive row KEYS; the FROM/JOIN/WHERE tail past the cap is matched case-insensitively, so bail-lowercasing it stays harmless.
- [x] **Step 2.2** Probe (`NormalizeQuery<Q_TeamPseInfo>`) confirms `"monthlySalesTarget"` and `"targetCurrency"` keep their case (`CASE_KEPT`). (String-shape probe — strict-mode-independent.)
- [x] **Step 2.3** `npm run typecheck` → only `_V26` (line 450) errors; **no TS2589**. `--extendedDiagnostics`: Instantiations **10,362,513** vs 10,122,283 baseline (+2.4%, not a balloon; Phase 4 recovers it). Memory ~1.82 GB, ~11.7s.
...
- **2026-06-01 (session 2):** **Phase 1 DONE** — applied the `Rest`-recursion fix to the quote-aware path (`RewriteExtractRewriteOne`, both branches), added regression `MultiExtractWithLiteral` in `quote-aware-structural.test.ts` (confirmed RED-without/GREEN-with). **Phase 2 DONE** — `LowercaseOutsideQuotes` cap 500 → **800** (NOT 1500; see Step 2.1 — 1500 blows TS's ~1000 tail-recursion limit → mass TS2589). Case now preserved. State: only `_V26` (line 450) red; no TS2589; Instantiations 10.36M. Uncommitted, on `main` (per project workflow). **Stopped after Phase 2 at user request.**
  - **⚠️ Phase 3 RE-DIAGNOSIS NEEDED.** While prepping Phase 3 I mis-ran a type probe with `tsc file.ts` (a CLI file arg makes tsc IGNORE tsconfig → `strict`/`strictNullChecks` OFF → `string | null` collapses to `string`). Under that broken config the engine *looked* like it strips schema null universally. It does NOT — that was a probe artifact. The project is `strict: true`; re-run all Phase-3 nullability probes under the real config (`npx tsc --noEmit --strict scratch.ts`, or include the scratch in the project and run bare `tsc`). Bug #3 (LEFT/RIGHT/FULL adds `| null` ON TOP of the schema type) still stands and is unverified at strict. Do NOT trust any non-strict probe for nullability.
